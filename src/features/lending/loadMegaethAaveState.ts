...
const computeAssetPriceUsd = (priceMR: bigint, baseUnit: bigint): number | null => {
  if (baseUnit === 0n) return null;
  return safeNumber(priceMR) / safeNumber(baseUnit);
};
...
const computeSideRewardBreakdown = (
  ...
  if (totalSideUsd <= 0 || rewards.length === 0) return { apyTotal: 0, entries: [] };
  ...
  for (const reward of rewards) {
    ...
    apyTotal += apr;
    entries.push({ tokenSymbol: reward.rewardTokenSymbol, source: "incentives", apy: apr });
  }
...
export async function loadMegaethAaveState({ publicClient, network, user }: Params): Promise<MegaethAaveSnapshot> {
  ...
  const reserveAddresses = reservesData.map((r) => r.underlyingAsset);
  ...
  const userReservesByAsset = new Map<string, UserReserve>(
    userReserves.map((r) => [r.underlyingAsset.toLowerCase(), r])
  );
  ...
  reservesData.forEach((reserve, index) => {
    ...
    if (user !== null) {
      ...
      allowances[asset.id] = formatUnitsToNumber(allowancesRaw[index] ?? 0n, decimals);
    }
  });
